...
*/
      var i,
          ...
          sSliderHTML = '';
/*
      if (offer.DETAIL_PICTURE.RESIZE) {
        sSliderHTML += '<a class="product-item-detail-slider-image" data-dot="<span></span>" data-fancybox="gallery" data-caption="'+ this.config.title +'" href="'+ offer.DETAIL_PICTURE.SRC +'" data-entity="image" data-id="'+ offer.DETAIL_PICTURE.ID +'">'
            + '<img src="'+ (offer.DETAIL_PICTURE.RESIZE ? offer.DETAIL_PICTURE.RESIZE.src : offer.DETAIL_PICTURE.SRC) +'" alt="'+ this.config.alt +'" title="'+ this.config.title +'" itemprop="image">'
        + '</a>';
      }
*/
        ...
        sSliderHTML += '<a class="product-item-detail-slider-image" data-dot="<span></span>" data-fancybox="gallery" data-caption="'+ this.config.title +'" href="'+ images[i].SRC +'" data-entity="image" data-id="'+ images[i].ID +'">'
            + '<img src="'+ (images[i].RESIZE ? images[i].RESIZE.src : images[i].SRC) +'" alt="'+ this.config.alt +'" title="'+ this.config.title +'"'+ (i == 0 ? ' itemprop="image"' : '') +'>'
        + '</a>';
/*
				img = BX.create('IMG', {
					props: {
...
*/
      ...
      setTimeout(function() {
        $slider.trigger('refresh.owl.carousel').trigger('to.owl.carousel', [0, 0]);
      }, 250);
		...
		updatePrice: function(obPriceRow, price)
		{
          ...
          obPriceDiscount = this.getEntity(obPriceRow, 'price-discount');
			...
			if (obPriceCurrent)
			{
				if (price)
				{
					BX.adjust(obPriceCurrent, {html: !!BX.Currency ? BX.Currency.currencyFormat(price.RATIO_PRICE, price.CURRENCY, true) : price.PRINT_RATIO_PRICE});
/*
...
*/
						...
						if (obPriceDiscount)
						{
							...
							economyInfo = economyInfo.replace('#ECONOMY#', !!BX.Currency ? BX.Currency.currencyFormat(price.RATIO_DISCOUNT, price.CURRENCY, true) : price.PRINT_RATIO_DISCOUNT);
...
*/
    ...
    checkFavorite: function(){
      ...
      if (RS.Favorite === undefined) {
        return;
      }
      
      var obFavoriteText = this.getEntity(this.obFavoriteBtn, 'favorite-title'),
