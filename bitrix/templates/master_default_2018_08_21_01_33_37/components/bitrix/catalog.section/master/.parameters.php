...
/*
		'JS_DATA' => Json::encode(
            array_filter(
                CatalogSectionComponent::getTemplateVariantsMap(),
                function($arItem){
                    return in_array($arItem['VARIANT'], array(2,3,6));
                }
            )
        ),
*/
    ...
    $arTemplateParameters['PRODUCT_BLOCKS'] = array(
        'PARENT' => 'VISUAL',
        'NAME' => Loc::getMessage('RS.MASTER.PRODUCT_BLOCKS'),
        'TYPE' => 'LIST',
        'VALUES' => $arProductBlocks,
        'REFRESH' => 'Y',
        'MULTIPLE' => 'Y',
        'DEFAULT' => '-',
    );

    if (is_array($arCurrentValues['PRODUCT_BLOCKS']) && count($arCurrentValues['PRODUCT_BLOCKS']) > 0) {
        $selected = array();
        foreach ($arCurrentValues['PRODUCT_BLOCKS'] as $name)
        {
            if (isset($arProductBlocks[$name]))
            {
                $selected[$name] = $arProductBlocks[$name];
            }
        }
...
if (Loader::includeModule('redsign.master')) {
    ParametersUtils::addCommonParameters($arTemplateParameters, $arCurrentValues, array('blockName'));
}
