...
function updateBasketTable(basketItemId, res)
{
	...
	if (basketItemId !== null && !!res.BASKET_DATA)
	{
		...
		if (createNewItem)
		{
			...
			for (i = 0; i < arColumns.length; i++)
			{
				switch (arColumns[i])
				{
					...
					case 'NAME':
						...
						{
							imageURL = basketJSParams.TEMPLATE_FOLDER + '/images/no_photo.jpg';
						}
            ...
            oCellQuantityHTML += '<div class="product-item-detail-amount product-item-amount dropdown">';
                oCellQuantityHTML += '<div class="btn btn-lg btn-gray dropdown-toggle" data-toggle="dropdown" style="position:relative;">';
                    oCellQuantityHTML += '<input class="product-item-amount-field" type="text" id="QUANTITY_INPUT_' + arItem["ID"] + '" name="QUANTITY_INPUT_' + arItem["ID"] + '" maxlength="18" value="' + arItem["QUANTITY"] + '" onchange="updateQuantity(\'QUANTITY_INPUT_' + arItem["ID"] + '\', ' + arItem["ID"] + ', ' + ratio + ', ' + bUseFloatQuantity + ')">';
                    oCellQuantityHTML += '<div style="display: inline" class="product-item-amount-measure">' + BX.util.htmlspecialchars(arItem['MEASURE_TEXT']) + '</div>';

                oCellQuantityHTML += '</div>';
                oCellQuantityHTML += '<ul class="dropdown-menu">';
                for (var n = 1; n < 10; n++) {
                  oCellQuantityHTML += '<li><a class="product-item-amount-custom" href="javascript:;">' + n + '</a></li>';
                }
                oCellQuantityHTML += '</ul>';
            oCellQuantityHTML += '</div>';
            ...
            if (arItem['DISCOUNT_PRICE_PERCENT'] > 0) {
              oCellPrice.innerHTML = '<div class="product-item-price-current discount" id="current_price_' + arItem['ID'] + '">' + arItem['PRICE_FORMATED'] + '</div>' +
                '<div class="product-item-price-old" id="old_price_' + arItem['ID'] + '">' + fullPrice + '</div>';
            } else {
              oCellPrice.innerHTML = '<div class="product-item-price-current" id="current_price_' + arItem['ID'] + '">' + arItem['PRICE_FORMATED'] + '</div>';
            }
					...
					default:
						...
						if (arColumns[i] == 'SUM')
							customColumnVal += '<div class="product-item-price-current" id="sum_' + arItem['ID'] + '">';
...
function showBasketItemsList(val)
{
	...
	BX.removeClass(BX("basket_toolbar_button_delayed"), "btn-primary");
	BX.removeClass(BX("basket_toolbar_button_subscribed"), "btn-primary");
	BX.removeClass(BX("basket_toolbar_button_not_available"), "btn-primary");
	...
	if (val == 2)
	{
		...
		if (BX("basket_items_delayed"))
		{
			...
			BX.addClass(BX("basket_toolbar_button_delayed"), "btn-primary");
	...
	else if(val == 3)
	{
		...
		if (BX("basket_items_subscribed"))
		{
			...
			BX.addClass(BX("basket_toolbar_button_subscribed"), "btn-primary");
	...
	else if (val == 4)
	{
		...
		if (BX("basket_items_not_available"))
		{
			...
			BX.addClass(BX("basket_toolbar_button_not_available"), "btn-primary");
	...
	else
	{
		if (BX("basket_items_list"))
		{
			...
			BX.addClass(BX("basket_toolbar_button"), "btn-primary");
...
BX.ready(function() {
  ...
  $(basketItems).on('click', '.product-item-detail-amount .dropdown-menu > li', function () {
    var $el = $(this);
