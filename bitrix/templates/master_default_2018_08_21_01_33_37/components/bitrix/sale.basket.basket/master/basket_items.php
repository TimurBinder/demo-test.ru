...
<div id="basket_items_list">
		...
		<table id="basket_items">
			<thead>
              ...
              <td></td>
						...
						?>
							<td class="price text-right" id="col_<?=$arHeader["id"];?>">
						<?
            elseif ($arHeader["id"] == "SUM"):
						?>
							<td class="custom text-right" id="col_<?=$arHeader["id"];?>">
						<?
						else:
						?>
				...
				foreach ($arResult["GRID"]["ROWS"] as $k => $arItem):
								...
								<td class="itemphoto">
									<div class="bx_ordercart_photo_container">
										...
										elseif (strlen($arItem["DETAIL_PICTURE_SRC"]) > 0):
											...
											$url = $templateFolder."/images/no_photo.jpg";
										endif;
                    ...
                    <div class="product-item-detail-amount product-item-amount dropdown">
                        <div class="btn btn-lg btn-gray dropdown-toggle" data-toggle="dropdown" style="position:relative;">
                            <input class="product-item-amount-field" type="text" id="QUANTITY_INPUT_<?=$arItem["ID"]?>" name="QUANTITY_INPUT_<?=$arItem["ID"]?>" maxlength="18" value="<?=$arItem["QUANTITY"]?>" onchange="updateQuantity('QUANTITY_INPUT_<?=$arItem["ID"]?>', '<?=$arItem["ID"]?>', <?=$ratio?>, <?=$useFloatQuantityJS?>)">
                            <?php if (isset($arItem["MEASURE_TEXT"])): ?>
                            <div style="display: inline" class="product-item-amount-measure"><?=htmlspecialcharsbx($arItem["MEASURE_TEXT"])?></div>
                            <?php endif; ?>
                            <svg class="dropdown__icon icon icon-svg"><use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#svg-chevron-down"></use></svg>
                        </div>
                        <ul class="dropdown-menu">
                            <?php for($i = 1; $i < 10; $i++): ?>
                            <li class="product-item-amount-var">
                                <a href="javascript:;"><?=($i * $ratio)?></a>
                            </li>
                            <?php endfor; ?>
                            <li><a class="product-item-amount-custom" href="javascript:;"><?=(10 * $ratio)?>+</a></li>
                        </ul>
                    </div>
									</div>
							...
							?>
								<td class="price">
										<div class="product-item-price-current<?php if (floatval($arItem["DISCOUNT_PRICE_PERCENT"]) > 0) echo ' discount';?>" id="current_price_<?=$arItem["ID"]?>">
											<?=$arItem["PRICE_FORMATED"]?>
										</div>
										<div class="product-item-price-old" id="old_price_<?=$arItem["ID"]?>">
							...
							?>
								<td class="custom">
									...
									if ($arHeader["id"] == "SUM"):
									?>
										<div id="sum_<?=$arItem["ID"]?>" class="product-item-price-current">
									<?
									endif;
							...
							<td class="control">
								...
								if ($bDeleteColumn):
									...
									<a href="<?=str_replace("#ID#", $arItem["ID"], $arUrls["delete"])?>"
										...
										<svg class="icon-svg"><use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#svg-cross"></use></svg><?=GetMessage("SALE_DELETE")?></a>
									<br />
									<?
								endif;
								if ($bDelayColumn):
								?>
									<a href="<?=str_replace("#ID#", $arItem["ID"], $arUrls["delay"])?>"><svg class="icon-svg"><use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#svg-delay"></use></svg><?=GetMessage("SALE_DELAY")?></a>
								<?
								endif;
								?>
							</td>
	...
	<div class="bx_ordercart_order_pay">
        ...
        <div class="input-group">
            <input type="text" class="form-control" id="coupon" name="COUPON" value="" onchange="enterCoupon();">
            <label class="input-group-btn">
                <a class="bx_bt_button bx_big btn btn-primary" href="javascript:void(0)" onclick="enterCoupon();" title="<?=GetMessage('SALE_COUPON_APPLY_TITLE'); ?>"><?=GetMessage('SALE_COUPON_APPLY'); ?></a>
            </label>
        </div>
			</div><?
...
?>
			...
			<table class="bx_ordercart_order_sum">
				...
				<?if ($arParams["PRICE_VAT_SHOW_VALUE"] == "Y"):?>
					<tr>
						...
						<td id="allSum_wVAT_FORMATED" class="product-item-price-current"><?=$arResult["allSum_wVAT_FORMATED"]?></td>
					</tr>
					<?
					...
					?>
						<tr style="display: <?=($showTotalPrice ? 'table-row' : 'none'); ?>;">
							...
							<td class="custom_t2 product-item-price-old" id="PRICE_WITHOUT_DISCOUNT">
				...
				<?endif;?>
					<tr>
						...
						<td class="product-item-price-current" id="allSum_FORMATED"><?=str_replace(" ", "&nbsp;", $arResult["allSum_FORMATED"])?></td>
					</tr>
      ...
      <a class="btn btn-lg btn-gray" id="clear_button"><?=GetMessage('SALE_CLEAR_BUTTON'); ?></a>
			<?if ($arParams["USE_PREPAYMENT"] == "Y" && strlen($arResult["PREPAY_BUTTON"]) > 0):?>
				<?=$arResult["PREPAY_BUTTON"]?>
				<span><?=GetMessage("SALE_OR")?></span>
			<?endif;?>
			<a href="javascript:void(0)" onclick="checkOut();" class="checkout btn btn-lg btn-primary"><?=GetMessage("SALE_ORDER")?></a>
		</div>
	</div>
</div>
<?
...
<div id="basket_items_list" >
          ...
          <div class="b-cart-empty">
              <div class="b-cart-empty__icon"><svg class="icon-svg"><use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#svg-cart"></use></svg></div>
              <h3 class="b-cart-empty__title"><?=GetMessage("SALE_NO_ITEMS")?></h3>
              <div class="b-cart-empty__note"><?=GetMessage("SALE_NO_ITEMS_NOTE")?></div>
          </div>
</div>
<?
endif;
