		...
		<tbody>
			...
			?>
                ...
                <?if (doubleval($arItem["DISCOUNT_PRICE_PERCENT"]) > 0):?>
									<div class="product-item-price-current discount"><?=$arItem["PRICE_FORMATED"]?></div>
									<div class="product-item-price-old"><?=$arItem["FULL_PRICE_FORMATED"]?></div>
								<?else:?>
									<div class="product-item-price-current"><?=$arItem["PRICE_FORMATED"];?></div>
								<?endif?>
						...
						?>
							<td class="custom<? if ($arHeader["id"] == "SUM") echo ' product-item-price-current';?>">
								<span><?=$arHeader["name"]; ?>:</span>
								<?=$arItem[$arHeader["id"]]?>
							</td>
						<?
					...
					?>
						<td class="control">
							<a href="<?=str_replace("#ID#", $arItem["ID"], $arUrls["add"])?>"><svg class="icon-svg"><use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#svg-cart"></use></svg><?=GetMessage("SALE_ADD_TO_BASKET")?></a><br />
							<?
							if ($bDeleteColumn):
							?>
								<a href="<?=str_replace("#ID#", $arItem["ID"], $arUrls["delete"])?>"><svg class="icon-svg"><use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#svg-cross"></use></svg><?=GetMessage("SALE_DELETE")?></a><br />
							<?
							endif;
							?>
						</td>
