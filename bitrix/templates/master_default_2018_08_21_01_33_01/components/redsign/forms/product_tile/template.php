    ...
    <form action="<?=$arResult['REQUEST_URI']?>" method="POST" class="form rsfrom__form" id="<?=$arResult['FORM_NAME']?>">
        ...
        <div class="rsform__all-field clearfix">
            ...
            <div class="row">
            	...
            	<?/* формирование селекта для цвета*/?>
				<?php if ($arField['CODE'] == 'COLOR' && is_array($arResult['EXT'])): ?>
                    <div class="form-group col col-md-12 field-wrap">
                        ...
                        <?php foreach ($arResult['EXT']['COLORS'] as $i => $arValue): ?>
                            <option data-image="<?=$arValue['UF_FILE_PATH']?>" value="<?=$arValue['UF_NAME']?>" data-price="<?=$arResult['EXT']['COLOR_PRICES'][$i]?>"><?=$arValue['UF_NAME']?> (<?=$arResult['EXT']['COLOR_PRICES'][$i]?>)</option>
                        <?php endforeach; ?>
                        </select>
                    </div>
...
<script>
$(function(){
	// отображение кол-ва товара в форме заказа
	var amount = $('.product-item-amount-field[type="tel"]').val();
	if (amount) {
		$('#FIELD_PRODUCT_QUANTITY').val(amount);	
	}
	else {
		$('#FIELD_PRODUCT_QUANTITY').val('1');	
	}
	
	// расчет итоговой цены
	var price = $('#FIELD_COLOR option:selected').attr("data-price");
	var quantity = Math.round($('#FIELD_PRODUCT_QUANTITY').val());
	$('#FIELD_PRICE').val(price * quantity);
