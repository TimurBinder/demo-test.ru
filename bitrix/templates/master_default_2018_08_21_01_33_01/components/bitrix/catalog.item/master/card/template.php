...
*/?>
	...
	<h2 style="height: auto;" class="product-item-title">
	...
	if (!empty($arParams['PRODUCT_BLOCKS_ORDER']))
	{
		foreach ($arParams['PRODUCT_BLOCKS_ORDER'] as $blockName)
		{
			...
			{
				case 'price': ?>
					<? /*Для раздела Тротуарная плитка цена не выводится*/ ?>
				...
				case 'buttons':
					...
					<div class="product-item-info-container" data-entity="buttons-block">
						...
						if (!$haveOffers)
						{
							...
							{
								?>
								<div style="text-align: center;" class="product-item-button-container js-buy" id="<?=$itemIds['BASKET_ACTIONS']?>" data-productid="<?=$actualItem['ID']?>">
									<a data-type="ajax" class="btn btn-primary <?=$buttonSizeClass?>"
										href="/include/forms/<?=$form_catalog?>/?element_id=<?=$actualItem['ID']?>" rel="nofollow">
						...
						if (!empty($item['DISPLAY_PROPERTIES']))
						{
							...
							<div class="product-item-info-container product-item-hidden" data-entity="props-block">
								<table width="100%" cellpadding="0" cellspacing="0" border="0">
									<tr class="product-item-properties">
										<?
										foreach ($item['DISPLAY_PROPERTIES'] as $code => $displayProperty)
										{
											?>
											<td<?/*=(!isset($item['PROPERTY_CODE_MOBILE'][$code]) ? ' class="hidden-xs"' : '')*/?>>
