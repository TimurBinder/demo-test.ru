...
JCSmartFilter.prototype.postHandler = function (result, fromCache)
{
	...
	if (!!result && !!result.ITEMS)
	{
		...
		if (!!modef && !!modef_num)
		{
			...
			if (result.FILTER_URL && hrefFILTER != undefined)
			{
...
JCSmartFilter.prototype.selectDropDownItem = function(element, controlId)
{
  ...
  var nodes = BX.findChild(element.parentNode.parentNode, {}, false, true);
  if (nodes.length > 0) {
    for (var i in nodes) {
  ...
  BX.addClass(element.parentNode, 'active');
...
JCSmartFilter.prototype.onLoadSuccess = function (result, data)
{
  ...
  BX(result.COMPONENT_CONTAINER_ID + '_section').innerHTML = processed.HTML;
  BX.ajax.processScripts(processed.SCRIPT);
  
  processed = BX.processHTML(data.sorter, false)
  BX(result.COMPONENT_CONTAINER_ID + '_sorter').innerHTML = processed.HTML;
  BX.ajax.processScripts(processed.SCRIPT);
...
$(document).ready(function(){
	...
	$bxfilter.find('.bx-filter-search > input').on('keyup', function(){
        ...
        $filterBlock = $(this).closest('.bx-filter-block'),
 				$scrollItems = $filterBlock.find('.bx-filter-param-label');
