	...
	<ul class="nav nav-tabs"><?
		foreach ($arData as $tabId => $arTab)
		{
			if (isset($arTab["NAME"]) && isset($arTab["CONTENT"]))
			{
				...
				$content .= '<div id="'.$id.'_cont" class=  "tab-pane fade'.($tabActive ? ' in active' : '').'">'.$arTab["CONTENT"].'</div>';
				$tabIDList[] = $tabId;
			}
		}
		unset($tabId, $arTab);
...
<?php
		...
		foreach ($arData as $tabId => $arTab)
		{
			if (isset($arTab["NAME"]) && isset($arTab["CONTENT"]))
			{
				...
				?><div id="<?=$id?>_cont" class="tab-pane fade<?=($tabActive ? ' in active' : '')?>"><?=$arTab["CONTENT"]?></div><?
				if($tabActive || $activeTabId == "")
					$activeTabId = $tabId;
