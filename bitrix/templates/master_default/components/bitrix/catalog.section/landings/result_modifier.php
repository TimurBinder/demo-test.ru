...
if (intval($arParams['LANDING_ELEMENT_ID']) > 0) {
  ...
  foreach ($arResult["ITEMS"] as $key=>$item) {
      foreach ($item["PROPERTIES"] as $pid => &$arProp) {      
          if((is_array($arProp["VALUE"]) && count($arProp["VALUE"])>0) || (!is_array($arProp["VALUE"]) && strlen($arProp["VALUE"])>0)) {
              $arResult["ITEMS"][$key]["DISPLAY_PROPERTIES"][$pid] = CIBlockFormatProperties::GetDisplayValue($item, $arProp, 'news_out');
          }
      }
  }
  ...
  if (isset($picts['VALUE']) && is_array($picts['VALUE'])) {
