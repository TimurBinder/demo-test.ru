...
<section class="catalog-section" data-entity="parent-container" id="<?=$arParams['TEMPLATE_AJAXID']?>">
	<!-- section-container -->
	...
	if ($arParams['ADD_CONTAINER'] == 'Y')
	{
		...
		<div class="l-section l-section--padding l-section-dark">
			<div class="container">
				...
				<?php
				if ($arParams['SHOW_PARENT_NAME'] == 'Y' && strlen($arParams['PARENT_NAME']) > 0)
				{
					?>
					<div class="l-section__head">
						<div class="section-head">
							<div class="section-head__title">
								<h2 data-entity="header" data-showed="false" style="display: none; opacity: 0;"><?=$arParams['PARENT_NAME']?></h2>
							</div>
						</div>
					</div>
					<?php
	...
	else
	{
		...
		{
			?>
			<h2 class="l-section__name" data-entity="header" data-showed="false" style="display: none; opacity: 0;"><?=$arParams['PARENT_NAME']?></h2>
			<?php
		}
	...
	{
		?>
		<div class="text-center" data-entity="pagination" data-pagination-num="<?=$navParams['NavNum']?>">
	...
	//endregion
	?>
	<div class="catalog-section__items" data-entity="<?=$containerName?>">
			<!-- items-container -->
			<?
			if (!empty($arResult['ITEMS']) && !empty($arResult['ITEM_ROWS']))
			{
				...
				if ($arParams['USE_OWL'] == 'Y')
				{
					$arSliderOptions = array(
						...
						'navText' => array(
							'<svg class="icon-svg icon-svg-arrow-thin-left2"><use xlink:href="#svg-arrow-thin-left"></use></svg>',
							'<svg class="icon-svg icon-svg-arrow-thin-right2"><use xlink:href="#svg-arrow-thin-right"></use></svg>'
						)
					);
					...
					?>
					<div data-slider="true" data-slider-options='<?=\Bitrix\Main\Web\Json::encode($arSliderOptions)?>' class="row product-item-list owl-master" style="margin:0">
				...
				foreach ($arResult['ITEM_ROWS'] as $rowData)
				{
					...
					?>
					<div class="row product-item-list <?=$rowData['CLASS']?>" data-entity="items-row">
						<?
				...
				if ($arParams['SHOW_ERROR_SECTION_EMPTY'] == 'Y')
				{
					...
					<div class="b-error-page">
						<div class="text-primary">
							<svg class="icon-svg" style="font-size:3.75rem"><use xlink:href="#svg-store"></use></svg>
						</div>
						<h3 class="b-error-page__title"><?=$arParams['MESS_ERROR_SECTION_EMPTY']?></h3>
						<div class="b-error-page__note"><?=Loc::getMessage('RS_MM_BCS_MASTER_ERROR_EMPTY_ITEMS2')?></div>
						<div class="b-error-page__btn">
							<a class="btn btn-default" href="<?=SITE_DIR?>"><?=Loc::getMessage('RS_MM_BCS_MASTER_HOME_PAGE')?></a>
						</div>
					</div>
					<?php
				}
...
<div class="text-center" data-entity="lazyload">
		...
		if ($showLazyLoad)
		{
			?>
			<!-- pagination-lazyload -->
			<div class="btn btn-default btn-lg" data-use="show-more-<?=$navParams['NavNum']?>" style="margin: 15px;">
...
<?php if ($arParams['ADD_CONTAINER'] == 'Y'): ?>
			</div> <?php // .l-section__main ?>
			<?php
			if ($arParams['RS_SECTION_SHOW_BUTTON'] == 'Y')
			{
				?>
				<div class="l-section__footer">
					<div class="section-footer">
						<div class="section-footer__more-button text-center">
							<a href="<?=$arParams['RS_SECTION_BUTTON_LINK']?>" class="btn btn-gray">
								<?=$arParams['RS_SECTION_BUTTON_NAME']?><svg class="icon-svg"><use xlink:href="#svg-arrow-thin-right"></use></svg>
							</a>
						</div>
					</div>
				</div>
