...
*/?>
		<div class="product-item-detail-info-section-link favorite-link js-favorite in-catalog" id="<?=$itemIds['FAVORITE_LINK_ID']?>">
			<span class="favorite-link__link"><i class="fa fa-heart <? if(in_array($item['ID'], $rsFavoriteElements['ID']) || in_array($item['ID'], $rsFavoriteElements)) { echo "green"; }?>"></i></span>
        </div>
	</div>
	<?php
	...
	<a class="product-item-image-wrapper" href="<?=$item['DETAIL_PAGE_URL']?>" title="<?=$imgTitle?>"
		...
		<span class="product-item-image-original" id="<?=$itemIds['PICT']?>"
			style="background-image: url('<?=$actualItem['PREVIEW_PICTURE']['SRC']?>');"></span>
<?/*
				...
				case 'price': ?>
					<div class="product-item-info-container product-item-price-container" data-entity="price-block">
						...
						<span
							...
							id="<?=$itemIds['PRICE']?>">
							<?
							$a = strrev($item['PROPERTIES']['PRICE']['VALUE']);
							$a = chunk_split($a, 3, ' ');
							//$a = strrev($a);
							echo strrev($a);?> руб.
							<?/*
							...
							if ($actualItem['CAN_BUY'])
							{
								...
								<span class="js-buy" id="<?=$itemIds['BASKET_ACTIONS']?>">
									...
									<a class="btn btn-default <?=$buttonSizeClass?>"
										id="<?=$itemIds['NOT_AVAILABLE_MESS']?>" href="<?=$item['DETAIL_PAGE_URL']?>">
