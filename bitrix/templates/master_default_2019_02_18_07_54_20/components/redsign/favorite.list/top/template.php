...
<a href="<?=$arParams['FAVORITE_URL']?>" class="b-topline-favorite" id="<?=$mainId?>">
    ...
    ?>
	<?/*<svg class="icon-svg"><use xlink:href="#svg-favorite-main"></use></svg>*/?>
	<i class="fa fa-heart" style="font-size: 1.5em;"></i>
		<span class="b-topline-favorite__count js-favorite-count"><?=$arResult['COUNT']?></span>
        <script>RS.Favorite.init(<?=\Bitrix\Main\Web\Json::encode($arIDs)?>)</script>
    <?php $frame->beginStub(); ?>
	<?/*<svg class="icon-svg"><svg class="icon-svg"><use xlink:href="#svg-favorite-main"></use></svg>*/?>
<i class="fa fa-heart" style="font-size: 1.5em;"></i>
		<span class="b-topline-favorite__count js-favorite-count"><?=$arResult['COUNT']?></span>
    <?php $frame->end(); ?>
</a>
