...
<div class="b-cart">
            ...
            <tbody>
                ...
                <tr data-id="<?=$arItem['ID']?>" data-product-id="<?=$arItem['PRODUCT_ID']?>">
                    <td class="b-cart__table-name" colspan="2">
                        ...
                        <?php endif; ?>
                        <a href="<?=$arItem['DETAIL_PAGE_URL']?>" class="b-cart__item-name"><?=$arItem['NAME']?> <?=($arItem['COLOR']?'('.$arItem['COLOR'].')':'')?></a>
                    </td>
					<?if($arItem['PRICE']==0):?>
					<td colspan=3> 
						По запросу
                    ...
                    <td>
                        <div class="product-item-detail-amount product-item-amount dropdown">
                            <div class="btn btn-gray dropdown-toggle" data-toggle="dropdown">
                                <input class="product-item-amount-field" onchange="rsCartSelectQuantity(<?=$arItem['ID']?>, $(this).val())" id="QUANTITY_INPUT_<?=$arItem['ID']?>" type="tel" value="<?=$arItem['QUANTITY']?>">
                                <?
								$measure = Loc::getMessage('RS_LIGHTBASKET_MEASURE_NAME');
								if(isset($arItem['PROPERTIES']['MEASURE']) && $arItem['PROPERTIES']['MEASURE']!='')$measure = $arItem['PROPERTIES']['MEASURE'];
								?>
								<span class="product-item-amount-measure"><?=$measure;?></span>
    ...
    <div class="b-cart__bottom">
        <a href="/catalog/" class="js-cart-close"><?=Loc::getMessage('RS_LIGHTBASKET_CONTINUE');?></a>
