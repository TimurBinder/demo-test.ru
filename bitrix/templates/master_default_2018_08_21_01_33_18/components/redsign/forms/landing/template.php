...
?>
<div class="rsform landing__form-transparent">
    ...
    <form action="<?=$arResult['REQUEST_URI']?>" method="POST" class="form rsfrom__form" id="<?=$arResult['FORM_NAME']?>">
        ...
        <div class="rsform__all-field clearfix">
            ...
            <div class="row">
                ...
                <div class="form-group col col-md-12 field-wrap">
                      <?$name = $arField['NAME'].($arField['IS_REQUIRED'] == 'Y' ? '*' : '');?>
                      <?php if ($arField['USER_TYPE'] == 'HTML'): ?>
                          <?php $arUserTypeSettings = unserialize($arField['USER_TYPE_SETTINGS']); ?>
                          <textarea id="FIELD_<?=$arField['CODE']?>" name="FIELD_<?=$arField['CODE']?>" class="form-control<?=$disabled?>" placeholder="<?=($arField['CURRENT_VALUE'] ? $arField['CURRENT_VALUE'] : $name);?>" style="height: <?=$arUserTypeSettings['height']?>px"<?php if ($arField['IS_REQUIRED'] == 'Y') echo ' required'?>></textarea>
                      <?php elseif ($arField['USER_TYPE'] == 'Date'): ?>
                          <input type="date" id="FIELD_<?=$arField['CODE']?>" name="FIELD_<?=$arField['CODE']?>" value="<?=$arField['CURRENT_VALUE']?>" placeholder="<?=$name?>" class="form-control<?=$disabled?>" <?=$disabled?><?php if ($arField['IS_REQUIRED'] == 'Y') echo ' required'?>>
                      <?php elseif ($arField['USER_TYPE'] == 'DateTime'): ?>
                          <input type="datetime-local" id="FIELD_<?=$arField['CODE']?>" name="FIELD_<?=$arField['CODE']?>" value="<?=$arField['CURRENT_VALUE']?>" placeholder="<?=$name?>" class="form-control<?=$disabled?>" <?=$disabled?><?php if ($arField['IS_REQUIRED'] == 'Y') echo ' required'?>>
                      <?php else: ?>
                          <input <?php if(isset($arField['MASK'])) echo 'data-inputmask="\'mask\': \''.$arField['MASK'].'\'"'; ?> type="<?=$arField['INPUT_TYPE']?>"<?php if(!empty($arField['PATTERN'])) echo ' pattern="'.$arField['PATTERN'].'"'; ?> id="FIELD_<?=$arField['CODE']?>" name="FIELD_<?=$arField['CODE']?>" placeholder="<?=$name?>" value="<?=$arField['CURRENT_VALUE']?>" class="form-control<?=$disabled?>"<?=$disabled?><?php if ($arField['IS_REQUIRED'] == 'Y') echo ' required'?>>
                      <?php endif; ?>
                </div>
        ...
        <div class="rsform__bottom">
            <div class="rsform__bottom-button">
                <?php if ($arParams["FORM_ID"] != ""):?>
                    <input type="hidden" name="FORM_<?=$arParams['FORM_ID']?>" value="Y" />
                <?php endif;?>
                <input type="hidden" name="PARAMS_HASH" value="<?=$arResult['PARAMS_HASH']?>">
                <input type="submit" class="btn btn-primary landing__button-rounded" name="submit" value="<?=Loc::getMessage('MSG_SUBMIT')?>">
            </div>
