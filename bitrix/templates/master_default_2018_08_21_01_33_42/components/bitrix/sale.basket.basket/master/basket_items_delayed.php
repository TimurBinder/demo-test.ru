		...
		<tbody>
			...
			?>
                ...
                <?if (doubleval($arItem["DISCOUNT_PRICE_PERCENT"]) > 0):?>
					...
					?>
						<td class="control">
							<a href="<?=str_replace("#ID#", $arItem["ID"], $arUrls["add"])?>"><svg class="icon-svg"><use xlink:href="#svg-cart"></use></svg><?=GetMessage("SALE_ADD_TO_BASKET")?></a><br />
							<?
							if ($bDeleteColumn):
							?>
								<a href="<?=str_replace("#ID#", $arItem["ID"], $arUrls["delete"])?>"><svg class="icon-svg"><use xlink:href="#svg-cross"></use></svg><?=GetMessage("SALE_DELETE")?></a><br />
							<?
							endif;
							?>
						</td>
