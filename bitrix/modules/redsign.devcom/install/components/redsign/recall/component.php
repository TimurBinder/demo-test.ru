...
if (!function_exists('redsign_add_recall_type')) {
    function redsign_add_recall_type(): bool
    {
        $return = false;
        $et = new \CEventType();
        $EventTypeID = $et->Add([
        ...
        if ($EventTypeID > 0) {
            COption::SetOptionInt('redsign.devcom', 'installed_event_type_id', $EventTypeID);
            $arSites = array();
            ...
            if ($EventTemplateID > 0) {
                COption::SetOptionInt('redsign.devcom', 'installed_event_template_id', $EventTemplateID);
                $return = true;
            }
        } else {
            $return = false;
