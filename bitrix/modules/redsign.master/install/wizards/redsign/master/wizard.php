...
class SelectSiteStep extends CSelectSiteWizardStep
{
	function InitStep()
	{
		...
		$wizard->solutionName = "redsign.master";
	}
}
...
class SelectTemplateStep extends CSelectTemplateWizardStep
{
	...
	function OnPostForm()
	{
		...
		if ($wizard->IsNextButtonClick())
		{
			...
			if (in_array($templateID,  array("master")))
				$wizard->SetVar("templateID", "master");
		}
	}
	...
	function ShowStep()
	{
		...
		if (in_array("master", array_keys($arTemplates)))
		{
			$arTemplateOrder[] = "master";
		}

		$defaultTemplateID = COption::GetOptionString("main", "wizard_template_id", "master", $wizard->GetVar("siteID"));
		if (!in_array($defaultTemplateID, array("master"))) $defaultTemplateID = "master";
		...
		$arTemplateInfo = array(
			"master" => array(
				...
				"DESCRIPTION" => "",
				"PREVIEW" => $wizard->GetPath()."/site/templates/master/preview.gif",
				"SCREENSHOT" => $wizard->GetPath()."/site/templates/master/screen.gif",
			),
		);

	//	$this->content .= "<input type='hidden' value='redsign.master' name='templateID' id='templateID'>";//$this->ShowInputField('hidden', 'templateID', array("id" => "templateID", "value" => "al"));
...
class SelectThemeStep extends CSelectThemeWizardStep
{
	function InitStep()
	{
		...
		$this->SetNextStep("homepage_settings");
		$this->SetNextCaption(GetMessage("NEXT_BUTTON"));
	}
}
...
class SiteSettingsStep extends CSiteSettingsWizardStep
{
	function InitStep()
	{
		...
		$this->SetPrevStep("homepage_settings");

		$this->SetNextCaption(GetMessage("NEXT_BUTTON"));
		$this->SetTitle(GetMessage("WIZ_STEP_SITE_SET"));

		$siteID = $wizard->GetVar("siteID");
		$isWizardInstalled = COption::GetOptionString("redsign.master", "wizard_installed", "N", $siteID) == "Y";

		if(COption::GetOptionString("redsign.master", "wizard_installed", "N", $siteID) == "Y" && !WIZARD_INSTALL_DEMO_DATA)
        {
		...
		$wizard->SetDefaultVars(Array(
			// "siteLogo" => file_exists(WIZARD_SITE_PATH."include/logo.png") ? WIZARD_SITE_DIR."include/logo.png" : ($isWizardInstalled ? "" : "/bitrix/wizards/redsign/master/site/templates/master/assets/img/logo.png"),
			// "siteLogoRetina" => file_exists(WIZARD_SITE_PATH."include/logo_retina.png") ? WIZARD_SITE_DIR."include/logo_retina.png" : ($isWizardInstalled ? "" : "/bitrix/wizards/redsign/master/site/templates/master/assets/img/logo.png"),
		...
		$wizard->SetDefaultVars(
			Array(
				...
				"shopEmail" => COption::GetOptionString("redsign.master", "shopEmail", "sale@".$_SERVER["SERVER_NAME"], $siteID),
				"siteMetaDescription" => GetMessage("wiz_site_desc"),
				"siteMetaKeywords" => GetMessage("wiz_keywords"),
				// "installEshopApp" => COption::GetOptionString("redsign.master", "installEshopApp", "Y", $siteID),
			)
		);
...
class CatalogSettings extends CWizardStep
{
	function InitStep()
	{
		...
		$wizard->SetDefaultVars(
			Array(
				"catalogSubscribe" => (isset($arSubscribe[$siteID])) ? ($arSubscribe[$siteID]['use'] == "Y" ? "Y" : false) : "Y",//COption::GetOptionString("redsign.master", "catalogSubscribe", "Y", $siteID),
				"catalogView" => COption::GetOptionString("redsign.master", "catalogView", "list", $siteID),
...
class ShopSettings extends CWizardStep
{
	function InitStep()
	{
		...
		$wizard->SetDefaultVars(
			Array(
				"shopLocalization" => COption::GetOptionString("redsign.master", "shopLocalization", "ru", $siteID),
				"shopEmail" => COption::GetOptionString("redsign.master", "shopEmail", "sale@".$_SERVER["SERVER_NAME"], $siteID),
				"shopOfName" => COption::GetOptionString("redsign.master", "shopOfName", GetMessage("WIZ_SHOP_OF_NAME_DEF"), $siteID),
				"shopLocation" => COption::GetOptionString("redsign.master", "shopLocation", GetMessage("WIZ_SHOP_LOCATION_DEF"), $siteID),
				//"shopZip" => 101000,
				"shopAdr" => COption::GetOptionString("redsign.master", "shopAdr", GetMessage("WIZ_SHOP_ADR_DEF"), $siteID),
				"shopINN" => COption::GetOptionString("redsign.master", "shopINN", "1234567890", $siteID),
				"shopKPP" => COption::GetOptionString("redsign.master", "shopKPP", "123456789", $siteID),
				"shopNS" => COption::GetOptionString("redsign.master", "shopNS", "0000 0000 0000 0000 0000", $siteID),
				"shopBANK" => COption::GetOptionString("redsign.master", "shopBANK", GetMessage("WIZ_SHOP_BANK_DEF"), $siteID),
				"shopBANKREKV" => COption::GetOptionString("redsign.master", "shopBANKREKV", GetMessage("WIZ_SHOP_BANKREKV_DEF"), $siteID),
				"shopKS" => COption::GetOptionString("redsign.master", "shopKS", "30101 810 4 0000 0000225", $siteID),
				"siteStamp" => COption::GetOptionString("redsign.master", "siteStamp", $siteStamp, $siteID),

				//"shopCompany_ua" => COption::GetOptionString("redsign.master", "shopCompany_ua", "", $siteID),
				"shopOfName_ua" => COption::GetOptionString("redsign.master", "shopOfName_ua", GetMessage("WIZ_SHOP_OF_NAME_DEF_UA"), $siteID),
				"shopLocation_ua" => COption::GetOptionString("redsign.master", "shopLocation_ua", GetMessage("WIZ_SHOP_LOCATION_DEF_UA"), $siteID),
				"shopAdr_ua" => COption::GetOptionString("redsign.master", "shopAdr_ua", GetMessage("WIZ_SHOP_ADR_DEF_UA"), $siteID),
				"shopEGRPU_ua" =>  COption::GetOptionString("redsign.master", "shopEGRPU_ua", "", $siteID),
				"shopINN_ua" =>  COption::GetOptionString("redsign.master", "shopINN_ua", "", $siteID),
				"shopNDS_ua" =>  COption::GetOptionString("redsign.master", "shopNDS_ua", "", $siteID),
				"shopNS_ua" =>  COption::GetOptionString("redsign.master", "shopNS_ua", "", $siteID),
				"shopBank_ua" =>  COption::GetOptionString("redsign.master", "shopBank_ua", "", $siteID),
				"shopMFO_ua" =>  COption::GetOptionString("redsign.master", "shopMFO_ua", "", $siteID),
				"shopPlace_ua" =>  COption::GetOptionString("redsign.master", "shopPlace_ua", "", $siteID),
				"shopFIO_ua" =>  COption::GetOptionString("redsign.master", "shopFIO_ua", "", $siteID),
				"shopTax_ua" =>  COption::GetOptionString("redsign.master", "shopTax_ua", "", $siteID),

				"installPriceBASE" => COption::GetOptionString("redsign.master", "installPriceBASE", "Y", $siteID),
			)
		);
	}
	...
	function InitStep()
	{
		...
		if ($shopLocalization == "ua")
			...
			$wizard->SetDefaultVars(
				Array(
					"personType" => Array(
						"fiz" =>  COption::GetOptionString("redsign.master", "personTypeFiz", "Y", $siteID),
						"ur" => COption::GetOptionString("redsign.master", "personTypeUr", "Y", $siteID),
					)
				)
			);
	}
...
===*/
	...
	function ShowStep()
	{
		...
		if (
			...
			LANGUAGE_ID == "ru" &&
			(
				COption::GetOptionString("redsign.master", "wizard_installed", "N", $siteID) != "Y"
