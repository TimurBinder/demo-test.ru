...
if(COption::GetOptionString("redsign.master", "wizard_installed", "N", WIZARD_SITE_ID) == "Y" && !WIZARD_INSTALL_DEMO_DATA){
	if($wizard->GetVar('rewriteIndex', true) && $wizard->GetVar('siteLogoSet', true)){
		$iblockCode = "redsign_master_landings_".WIZARD_SITE_ID;
		$iblockType = "system";
		...
		{
            CWizardUtil::ReplaceMacros(WIZARD_SITE_PATH."/catalog/index.php", array("SYSTEM_LANDINGS_IBLOCK_ID" => $iblockID));
            CWizardUtil::ReplaceMacros(WIZARD_SITE_PATH."/news/index.php", array("SYSTEM_LANDINGS_IBLOCK_ID" => $iblockID));
            CWizardUtil::ReplaceMacros(WIZARD_SITE_PATH."/sale-promotions/index.php", array("SYSTEM_LANDINGS_IBLOCK_ID" => $iblockID));
            CWizardUtil::ReplaceMacros(WIZARD_SITE_PATH."/services/index.php", array("SYSTEM_LANDINGS_IBLOCK_ID" => $iblockID));
		}
	}
	return;
}
...
if (in_array(LANGUAGE_ID, array(/*'en', */'ru'))) {
    $iblockXMLFile = WIZARD_SERVICE_RELATIVE_PATH.'/xml/_all/'.$iblockType.'/landings-'.LANGUAGE_ID.'.xml';
} else {
	$iblockXMLFile = WIZARD_SERVICE_RELATIVE_PATH.'/xml/_all/'.$iblockType.'/landings-ru.xml';
}
...
WizardServices::IncludeServiceLang('system_landings.php', $lang);
...
CWizardUtil::ReplaceMacros(WIZARD_SITE_PATH."/catalog/index.php", array("SYSTEM_LANDINGS_IBLOCK_ID" => $iblockID));
CWizardUtil::ReplaceMacros(WIZARD_SITE_PATH."/company/staff/index.php", array("SYSTEM_LANDINGS_IBLOCK_ID" => $iblockID));
CWizardUtil::ReplaceMacros(WIZARD_SITE_PATH."/news/index.php", array("SYSTEM_LANDINGS_IBLOCK_ID" => $iblockID));
CWizardUtil::ReplaceMacros(WIZARD_SITE_PATH."/sale-promotions/index.php", array("SYSTEM_LANDINGS_IBLOCK_ID" => $iblockID));
CWizardUtil::ReplaceMacros(WIZARD_SITE_PATH."/services/index.php", array("SYSTEM_LANDINGS_IBLOCK_ID" => $iblockID));
