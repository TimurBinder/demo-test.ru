	...
	if ($arParams['FILL_ITEM_ALL_PRICES'])
	{
		...
		if ($arResult['MODULES']['redsign.master'])
		{
			foreach ($arResult['OFFERS'] as $iOfferKey => $arOffer)
			{
				...
				$arResult['JS_OFFERS'][$iOfferKey]['ITEM_ALL_PRICES'] = $arResult['OFFERS'][$iOfferKey]['ITEM_ALL_PRICES'];
			}
...
if ($arResult['DETAIL_TEXT'] != '')
{
	if ($arResult['DETAIL_TEXT_TYPE'] === 'html')
	{
		...
		{
			if (preg_match_all('#<iframe[^>]*class="[^"]*"[^>]*>[^<]*</iframe>#is', $arResult['DETAIL_TEXT'], $arMatches))
			{
				$arResult['DETAIL_TEXT'] = preg_replace(
					'#(<iframe[^>]*class="[^"]*)("[^>]*>[^<]*</iframe>)#is',
					'<div class="embed-responsive embed-responsive-16by9">$1 embed-responsive-item$2</div>',
					$arResult['DETAIL_TEXT']
				);
			}
			...
			if (preg_match_all('#(<iframe((?!class)[^>])*)(>[^<]*</iframe>)#is', $arResult['DETAIL_TEXT'], $arMatches))
			{
				$arResult['DETAIL_TEXT'] = preg_replace(
					'#(<iframe((?!class)[^>])*)(>[^<]*</iframe>)#is',
					'<div class="embed-responsive embed-responsive-16by9">$1 class="embed-responsive-item"$3</div>',
					$arResult['DETAIL_TEXT']
				);
			}
		}
	...
	if (substr($blockName, 0, 5) == 'prop_')
	{
		...
		if (!empty($arResult['PROPERTIES'][$sPropCode]['VALUE']))
		{
			if ($arResult['PROPERTIES'][$sPropCode]['PROPERTY_TYPE'] == 'S' && isset($arResult['DISPLAY_PROPERTIES'][$sPropCode]))
			{
				...
				{
					$arResult['DISPLAY_PROPERTIES'][$sPropCode]['DISPLAY_VALUE'] = preg_replace(
						'#<table[^>]*>[^>]*</table>#is',
						'<div class="table-responsive">$0</div>',
				...
				{
					if (preg_match_all('#<iframe[^>]*class="[^"]*"[^>]*>[^<]*</iframe>#is', $arResult['DISPLAY_PROPERTIES'][$sPropCode]['DISPLAY_VALUE'], $arMatches))
					{
						$arResult['DISPLAY_PROPERTIES'][$sPropCode]['DISPLAY_VALUE'] = preg_replace(
							'#(<iframe[^>]*class="[^"]*)("[^>]*>[^<]*</iframe>)#is',
							'<div class="embed-responsive embed-responsive-16by9">$1 embed-responsive-item$2</div>',
		...
		if ('F' == $arResult['PROPERTIES'][$sPropCode]['PROPERTY_TYPE'])
		{
