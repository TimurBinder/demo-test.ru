...
if(COption::GetOptionString("redsign.master", "wizard_installed", "N", WIZARD_SITE_ID) == "Y" && !WIZARD_INSTALL_DEMO_DATA){
	if($wizard->GetVar('rewriteIndex', true) && $wizard->GetVar('siteLogoSet', true)){
		$iblockCode = "redsign_master_reviews_".WIZARD_SITE_ID;
		$iblockType = "company";
		...
		{
            CWizardUtil::ReplaceMacros(WIZARD_SITE_PATH."/company/reviews/index.php", array("COMPANY_REVIEWS_IBLOCK_ID" => $iblockID));
            CWizardUtil::ReplaceMacros(WIZARD_SITE_PATH."/include/template/catalog/element_blocks.php", array("COMPANY_REVIEWS_IBLOCK_ID" => $iblockID));
            CWizardUtil::ReplaceMacros(WIZARD_SITE_PATH."/_index.php", array("COMPANY_REVIEWS_IBLOCK_ID" => $iblockID));
		}
	}
	return;
}
