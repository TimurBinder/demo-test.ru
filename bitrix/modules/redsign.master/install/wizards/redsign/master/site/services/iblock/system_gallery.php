...
if(COption::GetOptionString("redsign.master", "wizard_installed", "N", WIZARD_SITE_ID) == "Y" && !WIZARD_INSTALL_DEMO_DATA){
	if($wizard->GetVar('rewriteIndex', true) && $wizard->GetVar('siteLogoSet', true)){
		$iblockCode = "redsign_master_gallery_".WIZARD_SITE_ID;
		$iblockType = "system";
		...
		{
            CWizardUtil::ReplaceMacros(WIZARD_SITE_PATH."/company/photogallery/index.php", array("SYSTEM_GALLERY_IBLOCK_ID" => $iblockID));
            CWizardUtil::ReplaceMacros(WIZARD_SITE_PATH."/include/template/catalog/element_blocks.php", array("SYSTEM_GALLERY_IBLOCK_ID" => $iblockID));
            CWizardUtil::ReplaceMacros(WIZARD_SITE_PATH."/_index.php", array("SYSTEM_GALLERY_IBLOCK_ID" => $iblockID));
		}
	}
	return;
}
