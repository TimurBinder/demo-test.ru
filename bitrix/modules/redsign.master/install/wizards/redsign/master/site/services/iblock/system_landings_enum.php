...
if(COption::GetOptionString("redsign.master", "wizard_installed", "N", WIZARD_SITE_ID) == "Y" && !WIZARD_INSTALL_DEMO_DATA){
	if($wizard->GetVar('rewriteIndex', true) && $wizard->GetVar('siteLogoSet', true)){
		$iblockCode = "redsign_master_landings_enum_".WIZARD_SITE_ID;
		$iblockType = "system";
...
if (in_array(LANGUAGE_ID, array(/*'en', */'ru'))) {
    $iblockXMLFile = WIZARD_SERVICE_RELATIVE_PATH.'/xml/_all/'.$iblockType.'/landings_enum-'.LANGUAGE_ID.'.xml';
} else {
	$iblockXMLFile = WIZARD_SERVICE_RELATIVE_PATH.'/xml/_all/'.$iblockType.'/landings_enum-ru.xml';
}
...
WizardServices::IncludeServiceLang('system_landings_enum.php', $lang);
...
// CWizardUtil::ReplaceMacros(WIZARD_SITE_PATH."/catalog/index.php", array("SYSTEM_LANDINGS_ENUM_IBLOCK_ID" => $iblockID));
