...
if ($item['LABEL'] || $arParams['SHOW_DISCOUNT_PERCENT'] === 'Y')
{
	...
	<span class="product-item-label-text <?=$labelPositionClass?>" id="<?=$itemIds['STICKER_ID']?>">
		...
		{
			?>
			<span class="product-item-label-text-item" id="<?=$itemIds['DSC_PERC']?>"
		...
		if (!empty($item['LABEL_ARRAY_VALUE']))
		{
			foreach ($item['LABEL_ARRAY_VALUE'] as $code => $value)
			{
				...
				?>
				<span class="product-item-label-text-item<?=(!isset($item['LABEL_PROP_MOBILE'][$code]) ? ' hidden-xs' : '')?>"<?if (strlen($sLabelStyle) > 0){ echo $sLabelStyle; }?> title=" <?=$value?>">
					<?=$value?>
				</span>
				<?
			}
