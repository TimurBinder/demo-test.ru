...
?>
<?php if ($arParams['USE_B24_CRM_FORM'] == 'Y'): ?>
<div id="<?=$arResult['FORM_NAME']?>" class="rsform rsform--crm"></div>
<script>
    b24form({"id": "<?=$arParams['B24_CRM_FORM_ID']?>", "lang": "<?=LANG?>", "sec": "<?=$arParams['B24_CRM_FORM_SEC']?>", "type":"inline", "node": document.getElementById('<?=$arResult['FORM_NAME']?>')});
</script>
<?php else: ?>
<div class="rsform">
            ...
            <div class="row">
                ...
                <div class="form-group col col-md-12 field-wrap">
                      ...
                      <?php if ($arField['USER_TYPE'] == 'HTML'): ?>
                          ...
                          <textarea id="FIELD_<?=$arField['CODE']?>" name="FIELD_<?=$arField['CODE']?>" class="form-control<?=$disabled?>"
                            style="height: <?=$arUserTypeSettings['height']?>px"<?php if ($arField['IS_REQUIRED'] == 'Y') echo ' required'?>
                            <?php if(isset($arField['PLACEHOLDER'])) echo 'placeholder="'.$arField['PLACEHOLDER'].'"'; ?>><?=$arField['CURRENT_VALUE']?></textarea>
                      ...
                      <?php else: ?>
                          <input <?php if(isset($arField['MASK'])) echo 'data-inputmask="\'mask\': \''.$arField['MASK'].'\'"';
                            ?> type="<?=$arField['INPUT_TYPE']?>"<?php if(!empty($arField['PATTERN'])) echo ' pattern="'.$arField['PATTERN'].'"';
                            ?> id="FIELD_<?=$arField['CODE']?>" name="FIELD_<?=$arField['CODE']?>" value="<?=$arField['CURRENT_VALUE']?>"
                            class="form-control<?=$disabled?>"<?=$disabled?><?php if ($arField['IS_REQUIRED'] == 'Y') echo ' required'?>
                            <?php if(isset($arField['PLACEHOLDER'])) echo 'placeholder="'.$arField['PLACEHOLDER'].'"'; ?>>
                      <?php endif; ?>
                </div>
