...
class RSDevFunc
{
    public static function Init(array $arInit): void
    {
        if (!is_array($arInit)) {
            $arInit = [$arInit];
        }
    ...
    public static function GetDataForProductItem(array &$arItems, array $params = []): void
    {
    ...
    public static function getPriceMatrixEx(
        array &$arElement,
        int $filterQuantity = 0,
        array $arFilterType = [],
        string $VAT_INCLUDE = 'Y',
    ...
    public static function getElementPictures(array &$arItem, array $params, int $iLimit = 0): array
    {
        $iCount = 0;
        $arPics = [];
        ...
        foreach ($arElements as $iElementKey => $arElement) {
            ...
            if (
                '' != $params['ADDITIONAL_PICT_PROP'][$arElement['IBLOCK_ID']]
                ...
                if (
                    ...
                    $propertyValue = $arElement['PROPERTIES'][$params['ADDITIONAL_PICT_PROP'][$arElement['IBLOCK_ID']]]['VALUE'];
                    if (!is_array($propertyValue)) {
                        if ((int)$propertyValue > 0) {
                            $arElements[$iElementKey]['PROPERTIES'][$params['ADDITIONAL_PICT_PROP'][$arElement['IBLOCK_ID']]]['VALUE'] = CFile::GetFileArray($propertyValue);
                            foreach ($params['RESIZE'] as $iResize => $arResize) {
                        ...
                        foreach ($propertyValue as $iFileKey => $iFileId) {
    ...
    public static function GetNoPhoto(array $arSizes = []): array
    {
        $return = [];
    ...
    public static function BasketEndWord(int $num, string $end1 = '', string $end2 = '', string $end3 = ''): string
    {
        $end1 = $end1 ?: Loc::getMessage('RSDF.END_1');
        $end2 = $end2 ?: Loc::getMessage('RSDF.END_2');
        $end3 = $end3 ?: Loc::getMessage('RSDF.END_3');

        $status = [$end1, $end2, $end3];
        $array = [2, 0, 1, 1, 1, 2];

        return $status[ ($num % 100 > 4 && $num % 100 < 20) ? 2 : $array[($num % 10 < 5) ? $num % 10 : 5]]; // @phpstan-ignore-line
    }

    public static function DeviceDetect(): array
    {
        $return = array(
            "DEVICE" => "pc",
            //"OS" => "",
    ...
    public static function getHighloadBlockValues(array &$arProp): void
    {
        /*
        ...
        if (isset($PROP_CACHE[$arProp['ID']])) {
            ...
            while ($oneEnum = $propEnums->fetch()) {
                ...
                $arProp['VALUES'][$oneEnum['ID']] = array_combine(
                    array_map(
                        function ($sKey) {
                            return substr((string)$sKey, 0, 3) == 'UF_' ? substr_replace((string)$sKey, '', 0, 3) : $sKey;
                        },
                        array_keys($oneEnum)
                    ),
                    $oneEnum
