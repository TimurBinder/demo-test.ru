...
class RSDevFuncResultModifier
{
    ...
    public static function SaleBasketBasketSmall(array $arResult): array
    {
        ...
        if (CModule::IncludeModule("catalog") && is_array($arNewResult["ITEMS"]) && count($arNewResult["ITEMS"]) > 0) {
            ...
            $arNewResult["PRINT_FULL_PRICE"] = FormatCurrency(
                $arNewResult["FULL_PRICE"],
                $arNewResult["ITEMS"][0]["CURRENCY"]
            );
        }
        return $arNewResult;
    }

    public static function SearchTitle(array $arResult): array
    {
        $arNewResult = $arResult;
        if (CModule::IncludeModule('iblock') && CModule::IncludeModule('catalog')) {
            ...
            if (!empty($arNewResult['CATEGORIES'])) {
                foreach ($arNewResult['CATEGORIES'] as $arCategory) {
    ...
    public static function SearchPage(array $arResult): array
    {
        $arNewResult = $arResult;
        if (CModule::IncludeModule('iblock') && CModule::IncludeModule('catalog')) {
            $arIBlocks = array();
    ...
    public static function CatalogSmartFilter(array $arResult): array
    {
        $arNewResult = $arResult;
        ...
        foreach ($arNewResult['ITEMS'] as $key => $arItem) {
            if ($arItem['PROPERTY_TYPE'] == 'S' || $arItem['USER_TYPE'] == 'directory') {
                ...
                if (
                    !isset($arItem['USER_TYPE_SETTINGS']['TABLE_NAME'])
                    || empty($arItem['USER_TYPE_SETTINGS']['TABLE_NAME'])
                )
                    continue;
                ...
                foreach ($VALUES_EX as $arVal) {
                    if (
                        isset($arNewResult['ITEMS'][$key]['VALUES'][ $arVal['XML_ID']])
                        && is_array($arNewResult['ITEMS'][$key]['VALUES'][ $arVal['XML_ID']])
                    ) {
